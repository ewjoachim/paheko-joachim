...
def test_add_replacement_cheque_persists(admin_page: Page, module_url, reseed, seed):
    ...
    # Re-planning it goes through the update branch, which anchors on the child's
    # own reception date — frozen to the day it was created, i.e. today.
    row.locator('select[name="rc_month[]"]').select_option("10")  # Octobre
    admin_page.get_by_role("button", name="Enregistrer").click()
    admin_page.wait_for_load_state("domcontentloaded")

    created = datetime.date.today()
    year = created.year + (1 if 10 < created.month else 0)
    admin_page.goto(f"{module_url}/index.html?month={year}-10", wait_until="domcontentloaded")
    expect(admin_page.locator("table.list", has_text="CHQ-0300")).to_have_count(1)
